...
class Ansi {
    ...
    public static $remove_tags = array(
        '<black>'       => "",
        '<red>'         => "",
        '<green>'       => "",
        '<yellow>'      => "",
    ...
    public static function remove_tags($string)
    {
        if (static::$enabled === null) {
            static::$enabled = !static::isWindows() || static::isAnsiCon();
        }
        ...
        if (!static::$enabled) {
            // Strip tags (replace them with an empty string)
            return str_replace(array_keys(static::$remove_tags), '', $string);
        }

        // We always add a <reset> at the end of each string so that any output following doesn't continue the same styling
        $string .= '';
        return str_replace(array_keys(static::$remove_tags), static::$remove_tags, $string);
    }
